...
def Production_Series(EPG_sima_visit, EPG_sima_duration, EPG_radio_visit, EPG_radio_duration, \
                                 ...
                                 EPG_news_new, EPG_series_new, EPG_sport_new, EPG_kid_new, EPG_documentary_new):
    ...
    for i in range(len(list_dup)):    # len(list_dup)
        ...
        EPG_sima_title = EPG_sima_new.query("channel == @var_channel")
    ...
    list_dup = list_dup.reset_index()
    del list_dup['index']
    EPG_S_news_visit = pd.DataFrame()
    EPG_news_title = pd.DataFrame()
    for i in range(len(list_dup)):    # len(list_dup)
        var_channel = list_dup.loc[i, 'channel']
        var_program = list_dup.loc[i, 'program']
        EPG_news_title = EPG_news_new.query("channel == @var_channel")
        EPG_news_title = EPG_news_title.query("program == @var_program")
        if len(EPG_news_title) > 1:
            EPG_S_news_visit1 = pd.DataFrame()
            EPG_S_news_visit1['channel'] = EPG_news_title['channel']
            EPG_S_news_visit1['program'] = EPG_news_title['program']
            EPG_S_news_visit1['Visit'] = EPG_news_title['Visit']
            EPG_S_news_visit1['Date'] = EPG_news_title['Date']
            EPG_S_news_visit1.sort_values('Date', axis = 0, ascending = True, inplace = True, na_position ='last')
            EPG_S_news_visit = pd.concat([EPG_S_news_visit, EPG_S_news_visit1])
    ###### news --- duration ######
    list_dup = EPG_news_duration.copy()
    list_dup.drop_duplicates(subset =['channel', 'program'], keep = 'first', inplace = True)
    list_dup = list_dup.reset_index()
    del list_dup['index']
    EPG_S_news_duration = pd.DataFrame()
    EPG_news_title = pd.DataFrame()
    for i in range(len(list_dup)):    # len(list_dup)
        var_channel = list_dup.loc[i, 'channel']
        var_program = list_dup.loc[i, 'program']
        ...
        EPG_news_title = EPG_news_title.query("program == @var_program")
        if len(EPG_news_title) > 1:
            EPG_S_news_duration1 = pd.DataFrame()
            EPG_S_news_duration1['channel'] = EPG_news_title['channel']
            EPG_S_news_duration1['program'] = EPG_news_title['program']
            EPG_S_news_duration1['Duration'] = EPG_news_title['Duration']
            EPG_S_news_duration1['Date'] = EPG_news_title['Date']
            EPG_S_news_duration1.sort_values('Date', axis = 0, ascending = True, inplace = True, na_position ='last')
            EPG_S_news_duration = pd.concat([EPG_S_news_duration, EPG_S_news_duration1])
    ###### series --- visit ######
    ...
    for i in range(len(list_dup)):    # len(list_dup)
        var_channel = list_dup.loc[i, 'channel']
        var_program = list_dup.loc[i, 'program']
        EPG_series_title = EPG_series_new.query("channel == @var_channel")
    ...
    for i in range(len(list_dup)):    # len(list_dup)
        var_channel = list_dup.loc[i, 'channel']
        var_program = list_dup.loc[i, 'program']
        EPG_sport_title = EPG_sport_new.query("channel == @var_channel")
    ...
    for i in range(len(list_dup)):    # len(list_dup)
        var_channel = list_dup.loc[i, 'channel']
        var_program = list_dup.loc[i, 'program']
        EPG_kid_title = EPG_kid_new.query("channel == @var_channel")
    ...
    for i in range(len(list_dup)):    # len(list_dup)
        var_channel = list_dup.loc[i, 'channel']
        var_program = list_dup.loc[i, 'program']
        EPG_documentary_title = EPG_documentary_new.query("channel == @var_channel")
    ...
    EPG_S_news_visit.to_excel(writer, 'اخبار-تعداد بازدید', index = False)
    EPG_S_news_duration.to_excel(writer, 'اخبار-زمان بازدید', index = False)
    ...
    return EPG_S_sima_visit, EPG_S_sima_duration, EPG_S_radio_visit, EPG_S_radio_duration, \
                                 ...
                                 EPG_S_news_visit, EPG_S_news_duration, EPG_S_series_visit, EPG_S_series_duration, EPG_S_sport_visit, EPG_S_sport_duration, \
